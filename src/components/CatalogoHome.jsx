  ...
  return (
    ...
    >
      ...
      <div className="text-center mb-20 px-6 z-10 w-full">
        ...
        <p className="reveal-up delay-100 text-xl md:text-2xl text-brand-green-dark/70 font-bold font-sans">
          Fios funcionais e luxuosos.
        </p>
      </div>

      <div className="container mx-auto px-6 md:px-12 w-full z-10">
        {/* O Grid Famoso "Float Flat" */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-10">
          {PRODUTOS.map((produto, index) => (
            <div
              key={produto.id}
              className={`reveal-up delay-${((index % 3) + 1) * 100} group cursor-pointer relative`}
            >
              {/* Fundo Arredondado */}
              <div
                className={`w-full h-[550px] ${produto.corBg} rounded-t-[5rem] rounded-b-[2rem] pt-12 px-6 flex flex-col items-center group-hover:-translate-y-4 transition-transform duration-500 ease-in-out`}
              >
                ...
                <span className="text-brand-green-dark/60 font-extrabold text-[11px] uppercase tracking-[0.25em] mb-3">
                  {produto.categoria}
                </span>
